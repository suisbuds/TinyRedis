...
func (k *KVStore) Expire(cmd *database.Command) handler.Reply {
	...
	expireAt := pkg.TimeNow().Add(time.Duration(ttl) * time.Second)
	_cmd := [][]byte{[]byte(database.CmdTypeExpireAt), []byte(key), []byte(pkg.TimeSecondFormat(expireAt))}
...
func (k *KVStore) ExpireAt(cmd *database.Command) handler.Reply {
	args := cmd.Args()
	key := string(args[0])
	expiredAt, err := pkg.ParseTimeSecondFormat(string((args[1])))
	if err != nil {
		return handler.NewSyntaxErrReply()
	}
	if expiredAt.Before(pkg.TimeNow()) {
		return handler.NewErrReply("ERR invalid expire time")
	}
...
func (k *KVStore) Set(cmd *database.Command) handler.Reply {
	...
	if affected > 0 && ttlStrategy {
		expireAt := pkg.TimeNow().Add(time.Duration(ttlSeconds) * time.Second)
		_cmd := [][]byte{[]byte(database.CmdTypeExpireAt), []byte(key), []byte(pkg.TimeSecondFormat(expireAt))}
